...
class User(BaseModel, db.Model, UserMixin):
    ...
    def to_auth_dict(self):
        id_card = self.id_card[:6]+'*'*8+self.id_card[-4:]
        length = len(self.id_name)
        if len(self.id_name)<3:
            id_name = self.id_name[:1]+'*'
        else:
            id_name = self.id_name[:1]+'*'*(length-2)+self.id_name[-1:]
        return {
            'id_name': id_name,
            'id_card': id_card
        }
